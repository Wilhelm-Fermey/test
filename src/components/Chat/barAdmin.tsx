...
const SetPassword = ({password,open, setPassword}:{password:boolean,open:boolean, setPassword: React.Dispatch<React.SetStateAction<boolean>>}) => {
  ...
  return (
    <div className={`my-2 absolute border-2 -mx-4 rounded-xl w-5/12 left-0 top-full my-3" ${password? "" : "hidden" } ${open?"":"hidden"} `} >
      <div>
...
const ManageUsers = ({manage, open, setManage}:{manage:boolean, open:boolean, setManage:React.Dispatch<React.SetStateAction<boolean>>}) => {
  ...
  return (
    <div className={` h-40 y-2 absolute border-2 overflow-x-auto rounded-xl w-7/12 right-0 top-full my-3" ${manage? "" : "hidden" } ${open?"":"hidden"}  `} >
        ...
        <div className={` border rounded-lg p-1 h-1/4  m-1 flex justify-center items-center flex-row ${IsBan?'bg-red-200':''} `} >
          ...
          <div className={`flex flex-col text-xs border z-50 rounded-md bg-slate-100 overflow-auto h-full w-1/3 transition-all ${!BanMenu && 'hidden'} ${IsBan && 'hidden'}`}>
          ...
          <div className={`flex flex-col text-xs border z-50 rounded-md bg-slate-100 overflow-auto h-full w-1/3 transition-all ${!MuteMenu && 'hidden'} ${IsMute && 'hidden'}`}>
